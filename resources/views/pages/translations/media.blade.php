...
@section('title')
Add images or files: {{$element->name}}
@stop
...
@section('content')
            ...
            <form action="{{url('admin/media/upload')}}" class="dropzone" id="dropzoneForm">
                ...
                <div class="row">
                    <div class="fallback">
                        ...
                        <input name="mediable_type" type="hidden" value="App\{{$element->class}}" />
    ...
    <div class="mx-auto mt-5">
        <a class="btn btn-primary" href="{{url('admin/'.$element->directory)}}">Back to {{ucfirst($element->directory)}}</a><a class="btn btn-success ml-2" href="{{$element->uri}}">See {{$element->name}}</a>
    </div>
